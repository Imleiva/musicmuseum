...
const WebGLErrorFallback = ({ children }) => {
  ...
  useEffect(() => {
    ...
    console.log('WebGL Check - Screen size:', window.screen.width, 'x', window.screen.height);
    console.log('WebGL Check - Viewport:', window.innerWidth, 'x', window.innerHeight);
    ...
    return (
      <div style={{
        ...
        <p style={{ 
          ...
          {error || 'Este dispositivo no soporta WebGL, necesario para la experiencia 3D del museo.'}
        </p>
        <p style={{ 
          fontSize: '0.8rem', 
          opacity: 0.6,
